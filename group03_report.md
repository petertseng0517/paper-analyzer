...
**GitHub**：[github.com/petertseng0517/paper-analyzer](https://github.com/petertseng0517/paper-analyzer)
...
| 姓名 | 學校 | 學系 | 學號 |
|:----:|:----:|:----:|:----:|
| 莊千響 | 銘傳大學 | 人工智慧應用學系 | 11160196 |
| 葉耀斌 | 銘傳大學 | 人工智慧應用學系 | 11160171 |
| 張照均 | 國立東華大學 | 電機系 | 411423026 |
| 曾建瑋 | 國立東華大學 | 資訊工程學系 | 611421501 |
...
　　1.1 人文研究者的閱讀困境　／　1.2 專案目標　／　1.3 選擇本地端 LLM 的理由

[**二、** AI 工具與人文研究的關係](#二ai-工具與人文研究的關係)

　　2.1 輔助而非取代：工具的定位　／　2.2 AI 摘要的認識論問題
...
　　4.1 PDF 上傳　／　4.2 論文分析　／　4.3 即時進度顯示

　　4.4 多篇論文比較　／　4.5 記憶功能　／　4.6 其他 UI 功能

[**五、** Prompt 設計](#五prompt-設計)

　　5.1 分段重點提取 Prompt　／　5.2 總結生成 Prompt

　　5.3 多篇比較 Prompt　／　5.4 Prompt 設計的反思

[**六、** 實作說明](#六實作說明)

　　6.1 文字分段邏輯（chunker.py）　／　6.2 非同步任務管理（main.py）　／　6.3 前端 Markdown 渲染（App.jsx）

[**七、** 操作說明](#七操作說明)

　　7.1 環境需求　／　7.2 安裝與啟動　／　7.3 使用流程　／　7.4 專案展示影片

[**八、** 遇到的問題與限制](#八遇到的問題與限制)

　　8.1 技術問題與解決方式　／　8.2 已知限制

[**九、** 倫理反思](#九倫理反思)

　　9.1 AI 摘要與學術誠信　／　9.2 語言與文化偏差　／　9.3 資料隱私
...
```mermaid
flowchart TD
    A["使用者瀏覽器<br/>React + Vite"] -->|"HTTP / WebSocket"| B["FastAPI 後端<br/>Uvicorn"]
    B --> C["pdf_parser.py<br/>PyMuPDF 提取文字"]
    C --> D["chunker.py<br/>依章節標題切分段落"]
    D --> E["analyzer.py<br/>LLM 呼叫"]
    E -->|"REST :11434"| F["Ollama 本地端<br/>Qwen2.5:7b"]
```
...
![主介面全覽：左側分析書架、右側分析結果區](img/main-interface-overview.png)
...
```mermaid
flowchart TD
    A[上傳 PDF] --> B["提取全文文字<br/>PyMuPDF<br/>進度 10%"]
    B --> C["依章節標題切分段落<br/>chunker.py<br/>進度 20%"]
    C --> D["逐段提取重點<br/>extract_key_points<br/>進度 20–90%"]
    D --> E["彙整重點，生成總結<br/>generate_summary_and_review<br/>進度 95%"]
    E --> F["分析完成<br/>100%"]
```

![單篇分析完成畫面：論文摘要、優點 Pros、缺點 Cons 三區塊](img/analysis-result-page1.png)

### 4.3 即時進度顯示

- 分析期間以**進度條**視覺化顯示百分比
- 同時顯示當前步驟文字說明（如「正在分析第 3/35 段...」）
- 多篇論文可自由切換分頁，各自顯示獨立進度

![分析進行中：進度條顯示 26%，目前正在分析第 3/35 段](img/upload-and-progress.png)
...
以下以兩篇語言學論文為例，示範跨論文比較功能的實際輸出。兩篇論文皆出自國立台灣大學語言學研究所，涵蓋不同時期與方法論的研究取向：

| | 論文 | 年份 | 研究主題 |
|--|------|------|---------|
| 論文一 | *Empowering Elementary Learning* | 2025 | 利用 GPT-4 為台灣國小生成適齡國語課文與生字練習 |
| 論文二 | *Stance Classification on PTT Comments* | 2015 | 以詞彙庫對 PTT 留言進行立場二元分類，準確率 71% |

兩篇論文相差十年，分別代表傳統規則式 NLP 與大型語言模型兩個時代的研究方法，研究場域也從社群媒體轉至教育應用，是呈現學術典範轉移的鮮明對照組。

![跨論文比較進行中（95%）：系統正在比對兩篇論文的重點與異同](img/cross-paper-comparison-progress.png)

比較完成後，系統輸出摘要比較、優缺點比較表與總結推薦：

![比較結果：摘要比較、優缺點表、及總結推薦](img/cross-paper-comparison-result.png)

工具正確識別出論文一的核心貢獻（個人化教材生成、文化背景考量）與限制（文本品質一致性問題），以及論文二的特色（繁體中文短文本立場辨識、具有高度實用性與代表性的語料庫）與侷限（分項任務的流程尚未深入探討、研究範圍偏限特定版塊）。最終推薦論文一為優先深讀對象，理由是研究創新性與教育政策實用性更高。此推薦結果與人工閱讀判斷高度吻合，驗證了工具在初步文獻篩選上的有效性。
...
```mermaid
flowchart TD
    A["論文全文"] --> B["切分為 N 個段落<br/>chunker.py"]
    B --> C["段落 1"]
    B --> D["段落 2 … N"]
    C --> E["Prompt ①<br/>提取各段重點<br/>繁體中文條列式"]
    D --> E
    E --> F["重點彙整<br/>combined key points"]
    F --> G["Prompt ②<br/>生成摘要＋優缺點<br/>固定三區塊格式"]
    G --> H["Markdown 輸出"]
```
...
腳本會自動檢查 Ollama 是否執行，並同時啟動後端與前端。按 **Ctrl+C** 可同時停止所有服務。
...
以下影片為本專案的完整功能示範，包含 PDF 上傳、分析流程與多篇比較的實際操作畫面：

[Paper-Analyzer 專案展示影片](https://www.youtube.com/watch?v=5mql9TgK2uQ)

![[yt-paper-analyzer.png]]
...
| 模型語言偏差 | 模型對英文論文的繁體中文輸出有時混入簡體字；日後可加入後處理步驟將簡體字統一轉換為繁體，提升輸出一致性 |
...
**輸出語言的細節問題**：即使 Prompt 已明確指定「繁體中文」，模型在處理英文輸入時仍偶爾混入簡體字。這提醒我們，Prompt 指令並不總是被完整遵守，日後可在後端加入簡繁轉換的後處理步驟，從工程層面補足 Prompt 無法完全控制的輸出細節。
...
未來仍有持續優化的空間，例如目前系統輸出偶爾會出現簡體字，後續可進一步針對語言輸出進行調整，確保介面與內容一律以繁體中文呈現，提升使用者在地化的體驗。
...
| 曾建瑋 | 實作一鍵複製分析結果至剪貼簿；新增 start.py 一鍵啟動腳本；管理本專案的 GitHub Repository |
| 張照均 | 修改網頁介面及主題 |
